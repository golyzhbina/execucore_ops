...
traces_path_list = load_var_and_data(get_var_path("hilb_filtration__traces_path"))
out_path_list = []

for traces_path in traces_path_list:
    traces_path = Path(traces_path)
    traces = load_obj(traces_path)
    ftraces = TracesFilter.hilb_filter_traces(traces)
    out_obj_path = get_obj_path("hilb_filtration__out", traces_path.stem)
    dump_obj(ftraces, Path(out_obj_path))
    out_path_list.append(out_obj_path)

out_var_path = get_var_path("hilb_filtration__out")
