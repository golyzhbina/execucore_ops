...
from msm_lib.traces import TracesReader

traces_path_list = load_var_and_data(get_var_path("read_traces__filename"))
normalize = load_var_and_data(get_var_path("read_traces__normalize"))
out_path_list = []

for traces_path in traces_path_list:
    traces_path = Path(traces_path)
    ftraces = TracesReader.read_traces(traces_path, normalize)
    out_obj_path = get_obj_path("read_traces__out", traces_path.stem)
    dump_obj(ftraces, Path(out_obj_path))
    out_path_list.append(out_obj_path)

out_var_path = get_var_path("read_traces__out")
dump_var_and_data(out_var_path, out_path_list)