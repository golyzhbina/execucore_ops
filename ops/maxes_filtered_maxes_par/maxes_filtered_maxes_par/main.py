...
kernel_size = load_var_and_data(get_var_path("filter_maxes__kernel_size"))
radius = load_var_and_data(get_var_path("filter_maxes__radius"))
threshold_value = load_var_and_data(get_var_path("filter_maxes__threshold_value"))
maxes_path_list = load_var_and_data(get_var_path("filter_maxes__maxes_path"))
out_obj_path_list = []
out_var_path = get_var_path("filter_maxes__out")

for maxes_path in maxes_path_list:
    try:
        maxes_path = Path(maxes_path)
        maxes = load_obj(maxes_path)
        filtered_maxes = MaxProcessor.filtered_maxes(maxes, threshold_value, radius, kernel_size)
        out_obj_path = get_obj_path("filter_maxes__out", maxes_path.stem)
        dump_obj(filtered_maxes, Path(out_obj_path))
        out_obj_path_list.append(out_obj_path)
    except Exception as e:
        out_obj_path_list.append("-")
        print(e)
