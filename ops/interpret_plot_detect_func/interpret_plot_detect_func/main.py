...
def get_obj_path(map_name):
    global map_vars
    return  f"{HOME}/{map_vars.get(map_name, map_name)}/{map_name}.png"


from msm_lib import load_obj, dump_obj
from msm_lib.result_interpret import Interpretator

df_path = load_var_and_data(get_var_path("plot_detect_function__detect_function_path"))
thresh_hold = load_var_and_data(get_var_path("plot_detect_function__thresh_hold"))
out_obj_path = get_obj_path("plot_detect_function__out")
