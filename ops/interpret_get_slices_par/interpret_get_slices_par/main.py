...
cube_path_list = load_var_and_data(get_var_path("get_slice__cube_path"))
maxes_path_list = load_var_and_data(get_var_path("get_slice__maxes_path"))
rec_coords_on_net_path = load_var_and_data(get_var_path("get_slice__rec_coords_on_net_path"))
region_path = load_var_and_data(get_var_path("get_slice__region_path"))
out_var_path = get_var_path("get_slice__out")

rec_coords_on_net = load_obj(rec_coords_on_net_path)
...
for i in range(len(cube_path_list)):
    try:
        ...
        out_obj_path = get_obj_path("get_slice__out", maxes_path.stem)
        os.makedirs(out_obj_path, exist_ok=True)
        Interpretator.get_slices(maxes, 
                                cube,
                                region,
                                rec_coords_on_net,
                                Path(out_obj_path))
